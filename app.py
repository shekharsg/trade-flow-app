...
# =========================
DATA_FILE = "data/trade_major_crops.csv.xz"  # <-- ensure this file is committed (not LFS), ~<25 MB
...
def load_data(path=DATA_FILE) -> pd.DataFrame:
    if not os.path.exists(path):
        st.stop()  # Hard stop with a clear message below
    try:
        df = _read_any(path)
    except Exception as e:
        st.error(f"Failed to read data file '{path}'. "
                 f"Tip: commit a real CSV.XZ/Parquet to GitHub (not Git LFS). Error: {e}")
        st.stop()

    needed = ["Year","Category","Item","original_Item",
              "Source_Countries","Target_Countries","value_kg_n"]
    missing = [c for c in needed if c not in df.columns]
    if missing:
        st.error(f"Data file missing required columns: {missing}")
        st.stop()
...
# -------------------------
# Basemap: avoid external shapefiles
# -------------------------
# Use built-in Natural Earth from GeoPandas to avoid bundling shapefiles.
world = gpd.read_file(gpd.datasets.get_path("naturalearth_lowres"))[["name","geometry"]].rename(columns={"name":"ADMIN"})
...
def plot_sankey(df, country, year, crop, orig_crop, category, threshold, flow_type="export", palette_choice="Set3"):
    ...
    if flow_type == "export":
        tcolors = {imp: selected_palette[i % len(selected_palette)] for i, imp in enumerate(importers)}
        node_colors = ["#636efa" if n in exporters else tcolors[n] for n in all_nodes]
        link_colors = [tcolors[t] for t in df_filtered["target"]]
    else:
        scolors = {src: selected_palette[i % len(selected_palette)] for i, src in enumerate(exporters)}
        node_colors = [scolors[n] if n in exporters else "#0ed6c1" for n in all_nodes]
        link_colors = [scolors[s] for s in df_filtered["source"]]
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        node=dict(pad=24, thickness=20, line=dict(color="#333", width=1.7),
    ...
    fig.update_layout(
        title_text=f"📊 {country}: {crop} ({category}) — {'Exports' if flow_type=='export' else 'Imports'} {year}",
        font=dict(family="Open Sans, Arial", size=14, color="#222"),
        plot_bgcolor="#fff", paper_bgcolor="#fff", margin=dict(l=20, r=80, t=60, b=40), height=600
    )
...
# Show standardized and original together
pairs = (trade_df[["Item","original_Item"]].drop_duplicates()
...
target_selected = st.sidebar.selectbox("Select Importing Country",
...
pal_choice = st.sidebar.radio("Choose Sankey Color Palette", ("Set3","Dark2","Pastel1"), index=0)

st.sidebar.markdown("---")
with st.sidebar.expander("ℹ️ About this dashboard"):
    st.markdown(
        """
        This app reads a reduced dataset (**data/trade_major_crops.csv.xz**).
        Make sure that file is **committed to GitHub (not via Git LFS)**.
        """
    )
...
plot_trade_flow(year_selected, category_selected, item_selected, original_selected,
                source_selected, flow_type="export", partner=target_selected)

st.header("📤 Export Flows Sankey")
plot_sankey(trade_df, source_selected, year_selected, item_selected, original_selected,
